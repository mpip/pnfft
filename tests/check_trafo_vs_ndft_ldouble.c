...
static void compare_f(
    ...
    long double f_hat_sum, const char *name, MPI_Comm comm_cart_3d);
static long double random_number_less_than_one(
    void);
    ...
    )
{
  ...
  /* plan parallel NFFT */
  pnfft = pnfftl_init_guru(3, N, n, x_max, m,
      PNFFT_MALLOC_F_HAT | window_flag, PFFT_ESTIMATE,
      comm_cart_3d);

  /* initialize nodes */
  nodes = pnfftl_init_nodes(local_M, PNFFT_MALLOC_X | PNFFT_MALLOC_F);

  /* get data pointers */
  f_hat = pnfftl_get_f_hat(pnfft);
  f     = pnfftl_get_f(nodes);
  x     = pnfftl_get_x(nodes);
  ...
  time = -MPI_Wtime();
  pnfftl_trafo(pnfft, nodes, PNFFT_COMPUTE_F);
  time += MPI_Wtime();
  ...
  time = -MPI_Wtime();
  pnfftl_trafo(pnfft, nodes, PNFFT_COMPUTE_DIRECT | PNFFT_COMPUTE_F);
  time += MPI_Wtime();

  /* print timing */
  MPI_Reduce(&time, &time_max, 1, MPI_DOUBLE, MPI_MAX, 0, comm);
  pfftl_printf(comm, "direct pnfftl_trafo needs %6.2e s\n", time_max);
  ...
  /* free mem and finalize */
  pnfftl_free(f1);
  pnfftl_finalize(pnfft, PNFFT_FREE_F_HAT);
  pnfftl_free_nodes(nodes, PNFFT_FREE_X | PNFFT_FREE_F);
...
{
  pfftl_get_args(argc, argv, "-pnfft_local_M", 1, PFFT_PTRDIFF_T, local_M);
  pfftl_get_args(argc, argv, "-pnfft_N", 3, PFFT_PTRDIFF_T, N);
  pfftl_get_args(argc, argv, "-pnfft_n", 3, PFFT_PTRDIFF_T, n);
  pfftl_get_args(argc, argv, "-pnfft_np", 3, PFFT_INT, np);
  pfftl_get_args(argc, argv, "-pnfft_m", 1, PFFT_INT, m);
  pfftl_get_args(argc, argv, "-pnfft_window", 1, PFFT_INT, window);
  pfftl_get_args(argc, argv, "-pnfft_x_max", 3, PFFT_DOUBLE, x_max);
}
